    ...
    public static class XMLExt
    {
        ...
        public static async Task<Monumento> XMLMonumentoToMonumento(XMLMonumento xm)
        {
            ...
            if (xm == null)
            {
                ...
                RejectedRecords.Add($"Nombre: {xm.nombre}, Error: El objeto XMLMonumento no puede ser nulo");
                return null;
            }
            ...
            if (string.IsNullOrWhiteSpace(xm.nombre))  
            {
                ...
                RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Monumento sin nombre");
                return null;
            }
            ...
            if (string.IsNullOrEmpty(xm.calle) && (string.IsNullOrEmpty(longitudStr) || string.IsNullOrEmpty(latitudStr)))
            {
                ...
                RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Imposible generar datos faltantes");
                return null;
            }
            else
            {
                ...
                { 
                    ...
                    RepairedRecords.Add($"Nombre: {xm.nombre}, Reparado: Latitud generada");
                }
                if (string.IsNullOrEmpty(latitudStr))
                {
                    Console.WriteLine($"Error: Imposible generar latitud, monumento '{xm.nombre}' rechazado.");
                    RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Imposible generar latitud");
                    return null;
                }
                if (string.IsNullOrEmpty(longitudStr))
                {
                    ...
                    RepairedRecords.Add($"Nombre: {xm.nombre}, Reparado: Longitud generada");
                }
                if (string.IsNullOrEmpty(longitudStr))
                {
                    Console.WriteLine($"Error: Imposible generar longitud, monumento '{xm.nombre}' rechazado.");
                    RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Imposible generar longitud");
                    return null;
                }
                if (string.IsNullOrEmpty(xm.calle))
                {
                    ...
                    {
                        Console.WriteLine($"Error: Monumento '{xm.nombre}' tiene una latitud inválida, imposible generar datos, rechazado.");
                        RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Latitud inválida");
                        return null;
                    }
                    if (!double.TryParse(longitudStr.Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out double lon))
                    {
                        Console.WriteLine($"Error: Monumento '{xm.nombre}' tiene una longitud inválida, imposible generar datos, rechazado.");
                        RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Longitud inválida");
                        return null;
                    }
                    ...
                    RepairedRecords.Add($"Nombre: {xm.nombre}, Reparado: Dirección generada");
                }
            }
            ...
            if (string.IsNullOrEmpty(xm.codigoPostal?.ToString()))
            {
                ...
                if (string.IsNullOrEmpty(xm.codigoPostal))
                {
                    ...
                    RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Imposible generar código postal");
                    return null;
                }
                Console.WriteLine($"Exito generando codigo postal para '{xm.nombre}', continua en filtros.");
                RepairedRecords.Add($"Nombre: {xm.nombre}, Reparado: Código postal generado");
            }
            ...
            if (!int.TryParse(xm.codigoPostal, out int codigoPostal))
            {
                ...
                RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Código postal inválido");
                return null;  
            }

            if (codigoPostal != 0 && (codigoPostal < 1000 || codigoPostal > 52999))
            {
                Console.WriteLine($"Error: Monumento '{xm.nombre}' tiene un código postal fuera del rango válido: {codigoPostal}, rechazado.");
                RejectedRecords.Add($"Nombre: {xm.nombre}, Error: Código postal fuera del rango válido");
                return null;
            }
            ...
            if (codigoPostal.ToString().Length < 5 && !string.IsNullOrEmpty(codigoPostal.ToString()) )
            {
                ...
                Console.WriteLine($"Exito arreglando el codigo postal de '{xm.nombre}'");
                RepairedRecords.Add($"Nombre: {xm.nombre}, Reparado: Código postal arreglado");
            }
